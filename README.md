...
| `region` | INDEC statistical region (Pampeana, Noroeste, Noreste, Cuyo, Patagonia), derived from `provincia` in `R/00_comun.R` | Supplementary | INDEC |
...
| 3 | `R/03_shift_share.R` | Shift-share: Kirchnerism against Fernández and against Milei, with SAS and SRL merged into one closed-company category (157 SAS in the reference cohort carry pre-2017 constitution dates from transformed companies; taken as a base they gave the form a national growth factor of ×947 and dominated the structural mix) | Provincial differentials for two comparison periods |
...
- The simplified form did not open a new position in the space of organisations. On the first axis it sits where the two commercial forms it replaces already sat, at +0.44 against +0.43 and +0.38 on an axis whose dispersion is 0.946. The second axis opposes cooperatives and construction to the SA and real-estate activities, and there the simplified form stays next to the SRL (+0.39 against +0.18) whilst the SA moves away (−0.47). The first axis carries 83.3% of the inertia and the plane 89.7%.
...
- The spread between the five regional subcloud centroids on the first axis grows from 0.59 to 0.89 standard deviations of the cloud between the 1990s and Fernández, and collapses to 0.25 under Milei.
